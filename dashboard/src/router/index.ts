...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/:catchAll(.*)', redirect: { name: 'homepage' } },
  ],
  scrollBehavior(to) {
    if (to.hash) {
      try {
  ...
  if (to.matched[0]) {
    ...
    if (to.meta.stats && (to.meta.stats as any).params) {
      (to.meta.stats as any).params.forEach((name: any) => {
        ...
        p = p.replace(':' + name, Array.isArray(value) ? value[0] : value || '');
      });
    }
    // p = p.replaceAll('?', '');
    p = p.replace(/\?/g, "")
    ...
    if ((to.meta.stats as any).params.view === 'traces' && to.query.query) {
      try {
        const q = JSON.parse(!Array.isArray(to.query.query) ? to.query.query : "");
    ...
    if ((to.meta.stats as any).params.view === 'applications' && (to.meta.stats as any).params.report === 'Profiling' && to.query.query) {
      try {
        const q = JSON.parse(!Array.isArray(to.query.query) ? to.query.query : "");
    ...
    if ((to.meta.stats as any).params.view === 'applications' && (to.meta.stats as any).params.report === 'Tracing' && to.query.trace) {
      const [type, id, ts, dur] = (!Array.isArray(to.query.trace) ? to.query.trace : "").split(':');
      p += `${type}:${id ? 'id' : ''}:${ts !== '-' ? 'ts' : ''}:${dur}`;
    }
    if ((to.meta.stats as any).params.view === 'applications' && (to.meta.stats as any).params.report === 'Logs' && to.query.query) {
      try {
        const q = JSON.parse(!Array.isArray(to.query.query) ? to.query.query : "");
