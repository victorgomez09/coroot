  ...
  >
    <template #activator="{ on, attrs }">
      <v-btn v-on="on" plain outlined height="40" class="px-2">
        ...
        <span v-if="!small" class="ml-2">{{
          intervals.find((i) => i.active).text
        }}</span>
...
watch(watch, (v) => {
    if (!v) {
      ...
      const from = route.query.from || "now-1h";
      const to = route.query.to || "now";
      const iFrom = parseInt(from);
      const iTo = parseInt(route.query.to);
      this.from = isNaN(iFrom) ? from : format.date(iFrom, fmt);
      this.to = isNaN(iTo) ? to : this.$format.date(iTo, fmt);
      this.dates =
        isAbsolute(this.from) && isAbsolute(this.to)
          ? [this.from.split(" ")[0], this.to.split(" ")[0]]
          : [];
    },
})

function isRelative(t: string) {
  return /^now(-\d+[mhdw])?$/.test(t);
}

function isAbsolute(t: string) {
...
//     apply() {
//       this.menu = false;
//       const from = isRelative(this.from)
//         ? this.from
...
//       const to =
//         this.$route.query.to === "now" ? undefined : this.$route.query.to;
//       const selected = intervals.find(
//         (i) => i.query.from === from && i.query.to === to
//       );
//       if (selected) {
