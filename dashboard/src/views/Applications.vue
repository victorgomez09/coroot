...
const items = computed(() => {
    ...
    const apps = applications.filter((a) => filter.has(a.id)).map((a) => ({ ...a }));
    const names: any = {};
    apps.forEach((a) => {
...
const link = (id: any, report: any, query?: any) => {
    return { name: 'overview', params: { view: 'applications', id, report }, query: { ...query, ...utils.contextQuery() } };
};
</script>
...
<template>
    <div>
        ...
        <v-data-table dense class="table" :mobile-breakpoint="0" :items-per-page="50" :items="items"
            no-data-text="No applications found" :headers="[
                ...
                { value: 'upstreams', title: 'Upstreams', sortable: false, align: 'end' },
                { value: 'instances', title: 'Instances', sortable: false, align: 'end' },
                ...
                { value: 'disk_io_load', title: 'I/O load', sortable: false, align: 'end' },
            ...
            <template #item.errors="{ item: { id, errors: param } }">
                <router-link :to="link(id, 'SLO')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            ...
            <template #item.instances="{ item: { id, instances: param } }">
                <router-link :to="link(id, 'Instances')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            ...
            <template #item.cpu="{ item: { id, cpu: param } }">
                <router-link :to="link(id, 'CPU')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            <template #item.memory="{ item: { id, memory: param } }">
                <router-link :to="link(id, 'Memory')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            <template #item.disk_io_load="{ item: { id, disk_io_load: param } }">
                <router-link :to="link(id, 'Storage')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            ...
            <template #item.network="{ item: { id, network: param } }">
                <router-link :to="link(id, 'Net')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            <template #item.dns="{ item: { id, dns: param } }">
                <router-link :to="link(id, 'DNS')" class="value" :class="param.status">{{ param.value || '–'
                }}</router-link>
            </template>
            <template #item.logs="{ item: { id, logs: param } }">
                <router-link :to="link(id, 'Logs', { query: JSON.stringify({ source: 'agent', view: 'patterns' }) })"
                    ...
                    <v-sparkline v-if="param.chart" :value="param.chart.map((v: any) => (v === null ? 0 : v))" fill
                        smooth padding="4" :color="`blue ${vuetify.theme.dark ? '' : 'lighten-4'}`" height="30"
                        width="120" />
                </router-link>
            </template>
        </v-data-table>
