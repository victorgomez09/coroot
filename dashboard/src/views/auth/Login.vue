<template>
    <div class="form">
        ...
        <v-form v-model="valid" @submit.prevent="post" ref="form">
            ...
            <v-text-field outlined dense type="email" v-model="email" name="email" :rules="[validators?.notEmpty]"
                :disabled="set_admin_password" />

            <div class="font-weight-medium">Password</div>
            <v-text-field outlined dense type="password" v-model="password" name="password"
            ...
            <template v-if="set_admin_password">
                ...
                <v-text-field outlined dense type="password" v-model="confirm_password"
                    :rules="[validators?.notEmpty, (v) => v === password || 'passwords do not match']" />
            </template>
...
const post = () => {
    loading = true;
    error = '';
    const parsedForm = { email: email.value, password: password.value, action: '' };
